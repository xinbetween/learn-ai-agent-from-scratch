...
const chapter: Chapter = {
  ...
  blurb:
    "这门课把模型调用视为一个以请求为作用域、从消息数组到响应的函数。它的签名要扛住流式、重试、用量记账和取消；把这件事做对，决定了接下来二十三章读起来有多舒服。",
  ...
  sections: [
    {
      ...
      html:
        ...
        note(
          "key",
          "塑造了一切的那个性质",
          p(`<strong>让请求契约保持无状态。</strong> 这门课在每次调用时都把模型所需的上下文发进去，因此对话状态由应用掌握，而不是藏在不透明的服务端会话里。有些服务商提供已保存的对话或服务端状态；即便如此，你仍要核算模型实际处理的上下文，并保留足以恢复、审计和编辑一次运行的状态。`)
        ),
    },
    {
      id: "core-idea",
    ...
    },
    {
      ...
      html:
        fig({
          ...
          body: REQ_SVG,
          caption:
            `在这门课里，那条虚线返回路径就是记忆：应用把响应追加进自己的数组。保留完整历史会让请求逐轮变大，输入工作量也近似按轮次二次增长；压缩和缓存会改变这个权衡，但不会消除管理它的必要。`,
        }) +
        `<h3>四种角色，各自派什么用场</h3>` +
        ul([
