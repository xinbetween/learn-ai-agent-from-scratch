...
const ITEMS: Item[] = [
  { when: "1973", what: "The actor model (Hewitt et al.)", why: "Addressed entities with mailboxes and asynchronous messages—the core ideas agent runtimes later reuse.", ch: "c18" },
  { when: "1986", what: "Brooks: the subsumption architecture", why: "Intelligent behaviour from layered reactive loops rather than a world model. The argument that the loop is the architecture predates modern language models by decades." },
  { when: "1986 →", what: "Erlang and OTP", why: "Process isolation, supervision, and message passing made actor-style systems practical at scale.", ch: "c18" },
  ...
  { when: "2022-10", what: "ReAct (Yao et al.)", why: "Interleave reasoning and acting. Its Thought/Action/Observation format made the loop easy to inspect and reproduce.", ch: "c04" },
  { when: "2023-03", what: "Reflexion and Self-Refine", why: "Critique and revise. They motivated careful evaluation of feedback signals rather than assuming a critic always helps.", ch: "c10" },
  { when: "2023-03", what: "AutoGPT, BabyAGI", why: "Early autonomous-loop projects that made missing termination, verification, and context management conspicuous.", ch: "c12" },
  { when: "2023-06", what: "Native function calling", why: "Tool use becomes less dependent on parsing free-form text. Schema-constrained output builds on related machinery.", ch: "c03" },
  ...
  { when: "2024-12", what: "Anthropic: Building Effective Agents", why: "A concise practical account of composition patterns and the case for using the simplest design that works.", ch: "c11" },
...
export function timelinePage(): Page {
  return {
    ...
    html:
      ...
      `</div>` +
      note("", "What has not changed", p(`The loop. C04 uses the same basic interleaving of decision, action, and observation that ReAct made legible in 2022. The later entries add tools for managing the loop in a real environment—cost, context, failure, and trust—rather than making its fundamentals irrelevant.`)),
  };
}
