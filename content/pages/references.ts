...
const REFS: Ref[] = [
  ...
  { title: "pi", who: "Earendil Works", ch: "c04", url: "https://github.com/earendil-works/pi/tree/main/packages/agent", what: "An agent harness in TypeScript, which makes it the closest production reading to this course's own code. <code>agent-loop.ts</code> is the loop of C04 at production scale: hooks and a queue for steering where this course has branches, and termination by abort signal and host policy rather than a step budget." },
  { title: "pi: the coding agent", who: "Earendil Works", ch: "c24", url: "https://github.com/earendil-works/pi/tree/main/packages/coding-agent", what: "C24's subject, complete and in the same language. Worth reading for the tool surface in <code>core/tools/</code>: unique-match editing, a file mutation queue, and line-or-byte truncation." },
  { title: "pi: compaction", who: "Earendil Works", ch: "c05", url: "https://github.com/earendil-works/pi/tree/main/packages/agent/src/harness/compaction", what: "Compaction with a retention budget, branch summarisation, and file operations lifted out of the history and preserved across the summary boundary — the domain-specific summary C05 argues for, implemented." },
  { title: "pi: the cache warmer", who: "Earendil Works", ch: "c01", url: "https://github.com/earendil-works/pi/blob/main/packages/coding-agent/src/core/cache-warmer.ts", what: "Prompt caching treated as an expiring asset: refresh at 90% of the TTL, but only when the expected saving clears a dollar threshold. Read the constants and their comments; one records that per-session estimates did not beat a flat probability." },
  { title: "pi: project trust", who: "Earendil Works", ch: "c16", url: "https://github.com/earendil-works/pi/blob/main/packages/coding-agent/src/core/project-trust.ts", what: "Approval at the granularity of a workspace rather than an action, gating the project config that can load resources, install packages and run extensions." },
