...
**Twenty-seven chapters and two capstones. One `while` loop that becomes Claude Code.**
...
| 🎛 **Simulators you can break** | Twenty-seven real implementations running in the page. Overflow the context window, corrupt a tool result, push a tool registry to 50 and watch selection collapse. |
| ✅ **162 quiz questions** | Six graded per chapter, each with an explanation naming the section to reread. |
| 🟦 **Code that runs** | 29 self-contained TypeScript files. All 29 execute in CI on every push, offline, with a deterministic mock model. |
...
<td>C13, C25, C14–C16</td>
<td>Code execution &amp; sandboxing · <b>the action space</b> (code actions vs tool calls) · files, shell and <code>apply_patch</code> · MCP · human approval</td>
</tr>
<tr>
...
npm run check:code                                       # all 29 files
...
them, and `npm run check:code` proves it by executing all 29.
...
code/                      ALL chapter code lives here
  c00..c26_*.ts            one runnable file per chapter
...
Chapter ids are URLs, so C25 and C26 were appended rather than inserted, and
`content/chapters/index.ts` places them in reading order — beside C13 and before
C22 respectively. Ids therefore run out of order in the sidebar gutter, on
